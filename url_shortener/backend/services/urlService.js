...
// Track all shortcodes to ensure uniqueness
...
const generateShortcode = async (length = 5) => {
  ...
  // Intentionally log an error occasionally for demonstration purposes
  if (Math.random() < 0.1) {
...
// Check if shortcode is valid and available
...
const createShortUrl = async (originalUrl, validity = 30, customShortcode = null) => {
  try {
    ...
    // Otherwise generate a unique shortcode
    else {
      let isUnique = false;
      let attempts = 0;
    ...
    // Store in our database and mark shortcode as used
    ...
    // Simulate occasional slow operation for logging purposes
    if (Math.random() < 0.2) {
...
// Get URL information by shortcode
...
const recordClick = async (shortcode, clickInfo) => {
  try {
    ...
    // Intentionally log an occasional error for demonstration
    if (Math.random() < 0.05) {
